...
class _AddClothesScreenState extends State<AddClothesScreen> {
  ...
  // ---------------- 저장 ----------------
  void _saveClothing() {
    if (!_isValidRequired) return; // ★ 가드: 비활성 상태일 때 호출 방지
    // TODO: 저장 API 호출
    debugPrint(
      'Saved! gender=$gender, master=$masterCategory, sub=$subCategory, '
      'article=$articleType, baseColor=$baseColor, seasons=${seasons.join(',')}, usage=$usage',
    );
    Navigator.pop(context, true); // 저장 후 이전 페이지로
  }
                      ...
                      Center(
                        child: SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: _pickFromCamera, // ★ 버튼 탭 시 촬영/등록
